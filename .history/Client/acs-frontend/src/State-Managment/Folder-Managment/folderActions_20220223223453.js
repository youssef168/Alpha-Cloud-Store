export const FolderActions = {
    ...
    SET_MOVE_ID: 'SET_MOVE_FOLDER_ID',
}
...
export const FolderHelpers = {
    ...
    deleteFolderSuccess: (folderID, parentID) => ({ type: FolderActions.DELETE_FOLDER_SUCCESS, folderID, parentID}),
    ...
    // delete file helpers
    deleteFile: (fileID, parentID) => ({ type: FolderActions.DELETE_FILE, fileID, parentID }),
    deleteFileSuccess: (fileID, parentID ) => ({ type: FolderActions.DELETE_FILE_FAIL, fileID, parentID}),
    deleteFileFail: (fileID, parentID, error) => ({ type: FolderActions.DELETE_FILE_FAIL, fileID, parentID, error}),

    // Move File helpers
    moveFile: (fileID, parentID, folderId) => ({ type: FolderActions.MOVE_FILE, fileID, parentID, folderId }),
    moveFileSuccess: (fileID, from , to, item) => ({ type: FolderActions.MOVE_FILE_SUCCESS, fileID, from, to, item }),
    moveFileFail: (fileID, from , to, error) => ({ type: FolderActions.MOVE_FILE_FAIL, fileID, from, to, error }),

    // Set Move Helper
    setMoveID: (id, moveType, parentID) => ({ type: FolderActions.SET_MOVE_ID, id, moveType, parentID}),

    // Search File & Folder helpers
    search: (query) => ({ type: FolderActions.SEARCH, query }),
    searchSuccess: (query, results) => ({ type: FolderActions.SEARCH_SUCCESS, query, results }),
    searchFail: (query, error) => ({ type: FolderActions.SEARCH_FAIL, query, error})
}